...
var word, currentWord, guessedLeft, guessed;
...
document.querySelector('input').addEventListener('change', function(){
  if(this.value != '' && this.value != ' '){
    if(this.value.length > 1){
      ...
      document.querySelector('.word').querySelector('span').innerHTML = guessesLeft;
    }

  });

function fadeColour(color){
...
function finish(){
  ...
  // var rightGuesses = guesses.length - wrongGuesses;

  var rightGuesses = word.length;

  let score = Math.floor((rightGuesses / (rightGuesses + wrongGuesses)) * 100) || 100;

  alert('Congatulations! Score: ' + score + '%');

  stats.streak++;

  stats.streal.push(score);

  setScore();

  fadeColour('lightblue');
